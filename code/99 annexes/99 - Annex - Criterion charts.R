...
directories=gta25_setup(internal.name="annex - p. 3 & 4 - top - criterion charts",
                        in.dev=F,
                        author=NULL,
                        wipe.data=F,
                        wipe.figs=F)
...
if(run.calc){
  ...
  pre.2017$name[pre.2017$name=="United Kingdom of Great Britain and Northern Ireland"]<-"UK"
  ...
  write.xlsx(lines, file=paste0(output.path,"Data for Track Record charts.xlsx"), row.names=F)
  save(lines, file=paste0(data.path,"Criterion charts values.Rdata"))
}
load(paste0(data.path,"Criterion charts values.Rdata"))
