...
directories=gta25_setup(internal.name="annex - p. 3 & 4 - bottom - bar charts",
                        in.dev=F,
                        author=NULL,
                        wipe.data=F,
                        wipe.figs=F)
...
write.xlsx(bars, file=paste0(output.path,"Data for bar charts.xlsx"), row.names = F)
...
annex.bar.charts <- function(member=NULL) {
  ...
  # draw it
  png(paste0(output.path, g20.member.names[member],"_bottom_protectionist.png"), width=800, height=800/1.8, res=76, type="cairo")
  grid.draw(g)
  dev.off()
  
  cairo_ps(paste0(output.path, g20.member.names[member],"_bottom_protectionist.eps"), bg = "white", width=11, height=11/1.8, family="Open Sans")
  grid.draw(g)
  dev.off()
  ...
  # draw it
  png(paste0(output.path, g20.member.names[member],"_bottom_liberalising.png"), width=800, height=800/1.8, res=76)
  grid.draw(g)
  dev.off()
